...
function doOpenDouban() {
  ...
  showDialog.value = false;
  // 从link中提取豆瓣subject_id
  const match = item.link ? item.link.match(/subject\/(\d+)/) : null;
  const subjectId = match ? match[1] : (item.tmdbid || '');
  // 使用doubanapp/dispatch格式（与海报跳转一致），桌面端打开网页，移动端唤起APP
  const url = subjectId ? `https://www.douban.com/doubanapp/dispatch?uri=/movie/${subjectId}?from=mdouban&open=app` : (item.link || '');
        ...
        default: _withCtx(() => [
          _createVNode(_component_VCard, { rounded: "lg" }, {
            default: _withCtx(() => [
              ...
              _createVNode(_component_VDivider),
              _createVNode(_component_VCardActions, { class: "pa-3 pt-2" }, {
                default: _withCtx(() => [
                  _createVNode(_component_VBtn, {
                    variant: "tonal",
                    color: "primary",
...
const Dashboard = /*#__PURE__*/_export_sfc(_sfc_main, [['__scopeId',"data-v-434311f8"]]);
