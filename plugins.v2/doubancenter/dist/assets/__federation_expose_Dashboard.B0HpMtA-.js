...
function doOpenDouban() {
  ...
  if (isMobile && item.tmdbid) {
    // 移动端尝试跳转豆瓣APP，失败则打开网页
    const doubanUrl = `douban://douban.com/subject/${item.tmdbid}`;
    const fallbackUrl = link || `https://movie.douban.com/subject/${item.tmdbid}/`;
    // 通过iframe尝试唤起APP
    const iframe = document.createElement('iframe');
    iframe.style.display = 'none';
    iframe.src = doubanUrl;
    ...
    // 桌面端直接打开网页
    window.open(link || `https://movie.douban.com/subject/${item.tmdbid}/`, '_blank');
  }
}
...
const Dashboard = /*#__PURE__*/_export_sfc(_sfc_main, [['__scopeId',"data-v-1fdc483a"]]);
