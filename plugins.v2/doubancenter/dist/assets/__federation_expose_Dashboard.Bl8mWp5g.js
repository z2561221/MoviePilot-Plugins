...
async function refreshRss() {
  ...
  try {
    ...
    const keysParam = keys.length ? `?rank_keys=${keys.join(',')}` : '';
    const res = await getPluginApi(props.api, `refresh_rss${keysParam}`);
    if (res.success) {
      if (res.data) {
...
const Dashboard = /*#__PURE__*/_export_sfc(_sfc_main, [['__scopeId',"data-v-500e1428"]]);
