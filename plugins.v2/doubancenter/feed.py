...
def _check_observe(self, unique: str, history: List[dict]) -> bool:
    """检查观察期：条目首次出现未满观察期则跳过订阅
    返回True表示还在观察期内（跳过）
    """
    days = self._observe_days
    if days <= 0:
        return False
    now = datetime.datetime.now()
    for h in history:
        if h.get("unique") == unique:
            first_seen = h.get("time", "")
            if first_seen:
                try:
...
def _process_coming(self, url: str, rd: dict) -> None:
    ...
    for item in items:
        ...
        if _check_anti_cheat(self, mediainfo):
            continue
        # 防刷榜：观察期
        if _check_observe(self, unique, history):
            continue
        ...
        if _add_sub(self, mediainfo, meta, rank_key=rd["key"], rank_name=rd["name"]):
            ...
            history.append({"title": cn_title, "year": year, "wish_count": wish, "air_date": ad, "link": link, "tmdbid": mediainfo.tmdb_id, "poster": mediainfo.get_poster_image(), "time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "unique": unique})
            uh.add(unique)
...
def _process_general(self, url: str, rd: dict) -> None:
    ...
    for item in items:
        ...
        if _add_sub(self, mediainfo, meta):
            cn_title = mediainfo.title or title
            history.append({"title": cn_title, "link": link, "tmdbid": mediainfo.tmdb_id, "poster": mediainfo.get_poster_image(), "time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "unique": unique})
            uh.add(unique)
    self.save_data(data_key, history)


def _process_items(self, items: List[dict], source: str) -> None:
    data_key = f"rank_history_custom_{hash(source)}"
    history: List[dict] = self.get_data(data_key) or []
    uh = {i.get("unique") for i in history}
    for item in items:
        title, link, mtype, year = item.get("title", ""), item.get("link", ""), item.get("mtype", ""), item.get("year")
    ...
    for item in items:
        try:
            ...
            try:
                ...
                if year:
                    meta.year = str(year)
                meta.type = MediaType.TV if rank_key == "coming" else (MediaType.MOVIE if item.get("mtype") == "movie" else MediaType.TV)
                mediainfo = self.chain.recognize_media(meta=meta, mtype=meta.type)
                if mediainfo:
                    tmdbid = mediainfo.tmdb_id
                    poster = mediainfo.get_poster_image()
    ...
    for item in items:
        try:
            ...
            if not mediainfo:
                continue
            cn_title = mediainfo.title or title
            history.append({"title": cn_title, "year": year, "wish_count": item.get("wish_count", 0), "link": link, "tmdbid": mediainfo.tmdb_id, "poster": mediainfo.get_poster_image(), "time": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "unique": unique})
            uh.add(unique)
            new_count += 1
        except Exception as e:
