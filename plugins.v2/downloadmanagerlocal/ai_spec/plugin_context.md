...
- 无站点策略时只接管下载器总上限，不写单种限速；站点策略模式优先给实际上传任务分配额度，每个下载器全局最多同时探测两个零速有 Peer 任务，候选按站点 4/2/1 权重轮换并保持两轮。已有弹性真实上传时，每个探测任务目标为 8 KiB/s；没有弹性真实上传时，当前两个探测任务可共享剩余下载器额度，避免大量候选把额度稀释到 1 KiB/s。
...
- 站点策略中的单种需求以 qBittorrent 下载者数量、Transmission Peer 状态和实际上传速率综合判断；没有 Peer 且没有上传的空闲任务分配为 0，不参与额度竞争。探测名额在每个下载器范围全局选择，最多两个，站点优先级按 4/2/1 加权轮换并保持两轮。已有弹性真实上传时，探测任务合计最多预留 16 KiB/s；真实任务需求有限或完全没有真实上传时，当前两个探测任务可在下载器总上限与站点硬上限内接走剩余额度，发现实际上传后下一轮优先保留主额度。
...
- `upload_limit_state`
  - schema v1 的上传限速运行态，保存管理状态、下载器原始/最后写入设置、单种原始/最后写入设置、宽限截止时间、失败计数和最近状态摘要。
