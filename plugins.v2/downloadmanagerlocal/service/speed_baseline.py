...
def suggest_thresholds(
    samples: list[dict],
    *,
    trusted_speed_bps: float,
    min_samples: int,
) -> dict:
    """根据健康样本生成仅供展示的阈值建议，不修改任何配置。"""
    values = [sample for sample in samples if isinstance(sample, dict)]
    speeds = [
        speed
    ...
    durations = [
        duration
        for sample in values
        for duration in [_positive_number(sample.get("effective_seconds"))]
        if duration > 0
    ]
    ...
    recommendation = {
        ...
        "grace_minutes": None,
        "interval_seconds": 30,
        "consecutive_abnormal_samples": 2,
        "min_samples": required,
    }
    reference = _positive_number(trusted_speed_bps)
    if len(speeds) < required or not reference:
        return recommendation

    ratios = [reference / speed for speed in speeds]
    tolerance = _percentile(ratios, 0.95)
    recommendation["tolerance"] = round(min(3.0, max(1.5, tolerance)), 1)
    if durations:
        grace_minutes = math.ceil(_percentile(durations, 0.90) / 60.0)
        recommendation["grace_minutes"] = min(5, max(2, grace_minutes))
