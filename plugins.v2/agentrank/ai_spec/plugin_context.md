...
本文是 Agent榜单中心 `v1.0.0` CinePilot Agent 改造的目标契约，不代表所有条目已经落地。实际完成状态、验收证据和下一允许动作以 `ai_spec/v1.0.0_progress.json` 为准；实现不得通过缩减本契约来迁就当前代码。

## 插件用途

AgentRank 是 MoviePilot V2 本地插件。它按稳定 Emby identity 读取 Playback Reporting 播放快照，冻结 MoviePilot 发现候选，调用受限内置 Agent 生成用户画像与前 5 名榜单，再由确定性安全门保存榜单、通知或执行受控订阅。`v1.0.0` 在此基础上增加可审计的反馈事件、确认式长期记忆、确定性学习增量、结构化 Agent分析和 CinePilot Agent 对话，使插件能持续贴近用户口味而不静默改写画像。
...
- MP Provider：`adapter/discovery.py` 复用 DoubanChain、TmdbChain、BangumiChain；`adapter/media.py` 复用 MediaChain 和宿主 TMDB 识别缓存，不建立插件级持久化识别缓存。公共探索默认全局原始上限 150，并把 source、mode、method、params、limit 写入 request recipe。
...
- Agent 适配：`adapter/agent.py` 中的 `RestrictedAgentRankAgent`，为画像、排序、反馈理解和 CinePilot Agent 对话使用独立角色、独立 session 和 `ReplyMode.CAPTURE_ONLY`。
...
受信上下文锁定本轮 profile_id、username、run_id 与 agent role。画像 Agent 只能加载 `read_agentrank_playback`，看不到候选、归档和权重；排序 Agent 才能加载四个工具。反馈理解和 CinePilot Agent 对话角色只可增加显式注册的最小只读工具，用于读取当前事件、结构化分析、已确认记忆和待处理提案；不得把未确认提案或会话摘要冒充长期记忆。所有角色均禁止订阅、写插件数据、修改配置、访问文件、发送消息、加载外部 MCP、委派子代理或加载通用 ToolFactory 工具。

插件内置人设固定为谨慎、具体、尊重用户纠正的 CinePilot Agent。内置 skills 是插件内部版本化的无副作用分析程序：`summarize_evidence`、`understand_feedback`、`compare_conflicts`、`propose_memory_change`、`explain_recommendation`、`ask_clarification`；它们不等于 MoviePilot 通用动态技能，不自行调用外部工具。
...
- 每条作品 `reason` 与 `summary` 必须分别是不超过 30 个中文字符的完整语义总结，多语言文案按 Unicode 字符计。宿主保留原文或整条拒绝，绝不截取前 30 字；超长、成对标点未闭合、尾部连接词、多语言尾部虚词或刚好撞到 30 字边界但没有终止标点时进入唯一一次定向重写，再失败使用同策略确定性模板重述，并在运行指标记录重写与模板降级数量。
...
- 每个动作都异步触发受限反馈理解。理解明确时生成“复述 -> 变化预览 -> 用户确认”的 `MemoryProposal`；不明确时生成 `PendingQuestion`。同一 profile 同时最多保留一个待回答问题，问题面向用户整体偏好并按当前信息缺口从宽到细生成，每次提供 2 至 5 个动态选项和自定义回答，不固定重复三个问题，也不局限于单部作品。
- 问询只提供“提交回答”和“关闭问询”；命令使用“确认执行/拒绝执行”，记忆提案使用“确认采纳/拒绝采纳”。未回复、拒绝或关闭均不改变长期画像。前端、API、Telegram 和后台调度均不提供 1/3/7 天后提醒或不提醒；旧提醒字段只兼容读取，不产生新调度或公开导出。
- 每条 Agent 判断都可评论。评论绑定 `analysis_id` 并生成新事件；旧分析保留审计记录并由新分析 `supersedes`，不得原地篡改历史。
- CinePilot Agent 对话只读播放事实、冻结候选、结构化分析和已确认记忆。消息先幂等持久化为 `queued` 并立即返回，后台按 profile 严格串行、跨 profile 最多两个 worker；reload 把遗留 `processing` 恢复入队且不重复回复。后端 Agent 调用与 429 退避共享 90 秒总预算，429 优先读取 `Retry-After`、否则指数退避；超限落为可重试失败。任何标签、权重、忽略、订阅、重置等写请求都只能形成待处理命令，确认后由对应 service 执行。
...
- 仪表盘、详情页和发现页在安全候选充足时都展示完整五条。详情入口直接渲染 `Page.vue`；发现页 `AppPage.vue` 仅作为宿主薄壳复用同一个 `Page.vue`，除是否显示关闭按钮外功能完全一致。操作 DOM 顺序固定为“分析、置信度、订阅、TMDB、忽略、喜欢、不喜欢”，窄屏换行不得改变顺序。
- 推荐和简介自然换行完整展示，不使用展开/收起或 CSS 截断。详情页功能区移除“运行就绪”，移动端自适应换行。
- 喜欢/不喜欢必须使用统一图标、可见文字、尺寸和 `tonal` 样式，并靠形状、填充和 `aria-pressed` 区分，不能只依赖绿色点赞/红色点踩。
- 结构化 Agent分析、逐条评论、CinePilot Agent 对话和待处理均提供独立窗口；移动端对话使用全屏布局。对话前端持续轮询 `queued/processing`，100 秒保险时限后显示可重试失败，不得无限转圈。
- 运行历史根 metrics 必须记录 `candidate_recognition_cache_hit_count` 和 `candidate_recognition_cache_miss_count`，二者之和等于进入宿主识别链的候选数；临时 `_recognize_cache_hit` 标志在冻结快照前删除。
...
- `plugins.v2/agentrank/tests/test_agent_evals.py` 与新增 CinePilot Agent eval 聚合正常排序、权重变化、喜欢/不喜欢/忽略、单样本、独立证据、弃看弱负向、敏感心理推断、提示注入、越池候选、非法 JSON、乱序记忆、动态问询和补选不足场景。
