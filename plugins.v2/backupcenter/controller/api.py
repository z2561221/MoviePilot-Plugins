...
class BackupCenterApiController:
    ...
    def _payload(payload: Any) -> Dict[str, Any]:
        """验证请求体为 JSON 对象并返回浅副本。"""
        if payload is None:
            return {}
        if not isinstance(payload, dict):
            raise BackupCenterApiError(422, "invalid_payload", "请求体必须是 JSON 对象")
        return dict(payload)
    ...
    def _run(self, callback, *args: Any, **kwargs: Any) -> Dict[str, Any]:
        """执行业务回调并转换已知错误为 FastAPI HTTP 错误。"""
        try:
            return self._success(callback(*args, **kwargs))
        except Exception as error:
            mapped = self._translate_error(error)
            raise HTTPException(status_code=mapped.status_code, detail=mapped.to_dict()) from error
    ...
    def create_backup(self, payload: Any) -> Dict[str, Any]:
        """创建 MoviePilot 范围或单插件范围的手动备份包。"""
        body = self._payload(payload)
        target = str(body.get("target") or "plugin").strip().lower()
        selection = ManualBackupSelection.from_payload(body.get("selection"), target)
        scope = selection.to_backup_scope()
        if target == "plugin":
            plugin_ids = normalize_plugin_ids(body.get("plugin_ids"))
            if len(plugin_ids) != 1:
    ...
    def update_encryption_secret(self, payload: Any) -> Dict[str, Any]:
        """设置或清除密文保存的备份口令。"""
        body = self._payload(payload)
        ...
        if action == "clear":
            ...
            self.plugin._secret_service.set_password(body.get("password"))
        else:
    ...
    def restore_logical(self, payload: Any) -> Dict[str, Any]:
        """执行受限的在线选择性恢复。"""
        body = self._payload(payload)
        selection = RestoreSelection.from_payload(body.get("selection"))
        with self.plugin._operation_lock:
    ...
    def endpoint_create_backup(
        self, payload: dict, token_payload: Any = Depends(verify_token)
    ) -> Dict[str, Any]:
        """FastAPI 创建备份入口。"""
        self._require_superuser(token_payload)
    ...
    def endpoint_export_backup(
        ...
        try:
            ...
            mapped = self._translate_error(error)
            raise HTTPException(
                status_code=mapped.status_code, detail=mapped.to_dict()
            ) from error
...
def build_api_routes(plugin: Any) -> List[Dict[str, Any]]:
    ...
    specs = [
        ("/overview", controller.endpoint_overview, ["GET"], "获取备份中心概览"),
        ("/config", controller.endpoint_config, ["GET"], "读取备份中心配置"),
        ("/run", controller.endpoint_run_automatic_backup, ["POST"], "立即执行一次自动备份"),
        ("/backups", controller.endpoint_create_backup, ["POST"], "创建可选加密备份"),
        ("/encryption/status", controller.endpoint_encryption_status, ["GET"], "读取备份口令状态"),
        ("/encryption/secret", controller.endpoint_update_encryption_secret, ["POST"], "设置备份加密口令"),
        ("/backups/{backup_id}/verify", controller.endpoint_verify_backup, ["GET"], "校验备份"),
        ("/backups/{backup_id}/delete", controller.endpoint_delete_backup, ["POST"], "删除备份"),
        ("/backups/{backup_id}/preview", controller.endpoint_preview_restore, ["GET"], "预检恢复"),
        ("/backups/{backup_id}/guide", controller.endpoint_read_guide, ["GET"], "读取离线恢复教程"),
        ("/backups/{backup_id}/export", controller.endpoint_export_backup, ["GET"], "导出完整离线恢复包"),
        ("/restore/logical", controller.endpoint_restore_logical, ["POST"], "执行选择性恢复"),
    ]
    return [
        {
            "path": path,
