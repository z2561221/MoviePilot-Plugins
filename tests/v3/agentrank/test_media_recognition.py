...
MediaSource = media_module.MediaSource
...
def test_recognition_prefers_tmdb_id_and_rebuilds_display_fields():
    ...
    assert calls[0]["media_source"] == MediaSource.TMDB
    assert calls[0]["media_id"] == "900"
...
def test_recognition_maps_douban_id_to_tmdb_before_source_recognition():
    ...
    class FakeChain:
        ...
        def convert_media_identity(self, **kwargs):
            ...
            return {"media_source": MediaSource.TMDB, "media_id": "910"}

        def recognize_media(self, **kwargs):
            """返回按 TMDB ID 读取的标准剧集。"""
            calls.append(("recognize", kwargs))
            assert kwargs["media_source"] == MediaSource.TMDB
            assert kwargs["media_id"] == "910"
            return SimpleNamespace(
                media_source=MediaSource.TMDB,
                media_id="910",
                tmdb_id=910,
    ...
    assert calls[0][1]["media_id"] == "db-910"
...
def test_recognition_uses_v3_identity_pair_contract_for_anilist():
    """V3 宿主先转换 AniList 身份，再按 TMDB 身份对识别媒体。"""
    calls = []
    ...
    class FakeChain:
        ...
        def convert_media_identity(self, **kwargs):
            """把 AniList 身份转换为 TMDB 身份。"""
            calls.append(("convert", kwargs))
            return {"media_source": MediaSource.TMDB, "media_id": "654"}

        def recognize_media(self, **kwargs):
            """按 TMDB 身份对返回 AniList 对应剧集。"""
            calls.append(("recognize", kwargs))
            assert kwargs["media_source"] == MediaSource.TMDB
            assert kwargs["media_id"] == "654"
            return SimpleNamespace(
                media_source=MediaSource.TMDB,
                media_id="654",
                tmdb_id=654,
    ...
    assert [call[0] for call in calls] == ["convert", "recognize"]
    assert calls[0][1]["media_source"] == MediaSource("anilist")
    assert calls[0][1]["media_id"] == "321"
...
def test_enrich_cross_source_ids_adds_douban_id_for_tmdb_item():
    ...
    class FakeChain:
        def convert_media_identity(self, **kwargs):
            calls.append(kwargs)
            return {"media_source": MediaSource.Douban, "media_id": "douban-900"}
...
def test_enrich_cross_source_ids_relaxes_year_when_exact_tmdb_match_is_empty():
    ...
    assert calls[0] == (
        "exact",
        {
            ...
            "media_source": MediaSource.TMDB,
            "media_id": "901",
            "mtype": FakeMediaType.MOVIE,
        },
    )
    assert calls[1] == ("relaxed", "年份漂移新片", FakeMediaType.MOVIE, None)
...
def test_enrich_cross_source_ids_accepts_legacy_tmdb_alias():
    ...
    class FakeChain:
        def convert_media_identity(self, **kwargs):
            assert kwargs["media_source"] == MediaSource.TMDB
            assert kwargs["media_id"] == "902"
            return {"media_source": MediaSource.Douban, "media_id": "douban-902"}
