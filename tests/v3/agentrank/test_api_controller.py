...
def test_current_analysis_read_is_profile_authorized_and_revision_bound():
    ...
    assert response["reason"] == "结构化推荐理由"
    ...
    assert stale.value.detail == "推荐分析已更新，请刷新榜单后重试"
    assert stale.value.headers["X-AgentRank-Error-Code"] == "analysis_revision_conflict"
...
def test_logged_in_user_can_access_every_configured_profile_and_full_options():
    ...
    assert controller.endpoint_overview(HOME_PROFILE, token)["profile_id"] == HOME_PROFILE
    assert controller.endpoint_board(REMOTE_PROFILE, token)["profile_id"] == REMOTE_PROFILE
...
def test_regular_user_can_read_and_write_every_configured_profile():
    ...
    assert controller.endpoint_overview(HOME_PROFILE, allowed)["profile_id"] == HOME_PROFILE
    archived = controller.endpoint_archive(
        {"profile_id": HOME_PROFILE, "candidate_id": "tmdb:1"}, allowed
    )
    assert archived["changed"] is True

    assert controller.endpoint_profile(REMOTE_PROFILE, allowed)["profile_id"] == REMOTE_PROFILE
    assert controller.endpoint_profile(HOME_PROFILE, unmapped_same_name)["profile_id"] == HOME_PROFILE
    second_user_archive = controller.endpoint_archive(
        {"profile_id": HOME_PROFILE, "candidate_id": "tmdb:1"},
        unmapped_same_name,
    )
    assert second_user_archive["event"]["candidate_id"] == "tmdb:1"

    with pytest.raises(fastapi_module.HTTPException) as caught:
        controller.endpoint_profile("emby:unknown:user-9", allowed)
    assert caught.value.status_code == 404
    assert caught.value.detail == "画像身份不在已选 Emby 用户中"
    assert caught.value.headers["X-AgentRank-Error-Code"] == "unknown_profile"
...
def test_attribution_routes_accept_any_logged_in_user_for_configured_profile():
    ...
    assert controller.endpoint_attribution(HOME_PROFILE, allowed)["profile_id"] == HOME_PROFILE
...
def test_conversation_endpoints_allow_profiles_and_pass_actor_privilege():
    ...
    assert controller.endpoint_conversation(HOME_PROFILE, allowed, True)["status"]["profile_id"] == HOME_PROFILE
    assert controller.endpoint_conversation_status(HOME_PROFILE, allowed)["profile_id"] == HOME_PROFILE
    sent = asyncio.run(
    ...
    assert controller.endpoint_conversation(HOME_PROFILE, another_user)["status"]["profile_id"] == HOME_PROFILE
...
def test_pending_endpoints_allow_profiles_and_hide_actor_from_response():
    ...
    assert responded["item"]["status"] == "rejected"
    assert service.calls[0][2]["actor_id"] == "7"
    assert service.calls[1][1]["actor_id"] == "7"
    assert "actor_id" not in str(responded)

    assert controller.endpoint_pending_center(HOME_PROFILE, another_user)["profile_id"] == HOME_PROFILE
...
def test_data_export_allows_logged_in_users_and_reset_token_stays_actor_bound():
    ...
    assert exported["profile"]["summary"] == "画像"
    assert "retention_policy" in exported

    assert controller.endpoint_data_export(HOME_PROFILE, another_user)["profile_id"] == HOME_PROFILE
    ...
    assert caught.value.detail == "重置交互学习需要明确确认"
    assert caught.value.headers["X-AgentRank-Error-Code"] == "confirmation_required"
    ...
    assert caught.value.detail == "确认令牌不属于当前用户"
    assert caught.value.headers["X-AgentRank-Error-Code"] == "confirmation_forbidden"

    reset = controller.endpoint_reset_full(
        {"profile_id": HOME_PROFILE, "confirmation_token": confirmation_token}, owner
    )
    assert reset["mode"] == "full"
...
def test_unified_feedback_endpoint_returns_event_state_and_board_revision():
    ...
    assert created["event_status"] == "created"
    assert created["board_revision"] == 1
    assert created["memory_delta"] == {}
    assert duplicate["event_status"] == "duplicate"
    assert duplicate["event"]["event_id"] == created["event"]["event_id"]
    ...
    assert disliked["event"]["supersedes"] == created["event"][
        "event_id"
    ]
    assert disliked["board_changed"] is False
    assert disliked["current_count"] == 1
    assert disliked["queue_status"] == "retry_wait"
    ...
    assert cancelled["event"]["supersedes"] == disliked["event"][
        "event_id"
    ]
...
def test_analysis_comment_api_is_bearer_scoped_idempotent_and_nonblocking():
    ...
    assert created["event"]["kind"] == "analysis_comment"
    assert "comment" not in created["event"]
    assert duplicate["event_status"] == "duplicate"
    assert duplicate["event"]["event_id"] == created["event"]["event_id"]
    other_created = controller.endpoint_analysis_comment(
        {**payload, "idempotency_key": "analysis-comment-api-2"}, another_user
    )
    assert other_created["event_status"] == "created"
...
def test_regular_user_status_and_config_options_include_all_profiles():
    ...
    status = controller.endpoint_status(regular_user)
    ...
    options = controller.endpoint_config_options(regular_user)
