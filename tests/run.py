"""插件仓全量单测入口：v1/v2/v3 各自独立 pytest 会话运行，命令行参数透传给 pytest。

plugins/（v1）、plugins.v2/（v2）与 plugins.v3/（v3）可能存在同名插件包，同一进程无法同时加载，故各代在
...
def _run_generation(generation: str, extra_args: list) -> int:
    """在独立子进程运行某一代（v1/v2/v3）的全部用例；该代无用例则跳过、返回 0。"""
...
if __name__ == "__main__":
    extra = sys.argv[1:]
    exit_code = 0
    # v1/v2/v3 必须分会话；按代依次跑，保留首个非零退出码作为整体结果
    for generation in ("v3", "v2", "v1"):
