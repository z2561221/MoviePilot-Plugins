"""pytest 全局引导：按本次运行目标选择 v1/v2/v3 插件环境并装载网络守卫。

``tests/run.py`` 会把 v1/v2/v3 放到独立 pytest 进程中运行；这里据本次目标路径只注入
对应插件目录，避免同一进程同时加载各代同名包。
"""
...
def _selected_generation(config) -> str:
    """根据 pytest 本次目标路径判断插件代际，禁止同一进程混跑多代插件。"""
    generations = set()
    ...
    raise RuntimeError("插件仓单测必须按 tests/run.py 分 v1/v2/v3 独立会话运行，避免同名插件包冲突")
