...
这个仓库里的插件，很多是靠对话推演、靠日志观测、靠一次次实验修出来的。说得直白一点，就是“主要靠嘴写，跑起来再用证据校正”。我会尽量让它们能用、少炸、出问题也有迹可循，但这不等于它们在每个 MoviePilot 环境里都绝对可靠。

所以，安装或更新前请自己看一眼代码和配置，重要数据先备份，最好在可回滚的环境里试运行。插件如果抽风、漏订、误订、页面不显示，先看日志、配置和版本差异；不要第一时间把锅全扣到仓库头上。哼，这不是免责声明偷懒，只是最基本的实验精神。
