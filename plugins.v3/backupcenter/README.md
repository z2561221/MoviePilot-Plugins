...
`v3.0.6`：修复恢复准备失败误回滚；插件 SQLite 数据库使用一致性快照。

## 职责边界

- 插件目录内的 SQLite 数据库通过只读在线备份接口生成独立一致性快照，不复制活动 WAL/SHM；PostgreSQL 插件 schema 仍由宿主数据库恢复点保护。
...
- 在线恢复：只替换备份中实际存在的设置、`PluginData` 和插件标准数据目录；停止目标后创建快照，只有实际开始写入的范围才在失败时回滚，完成后 reload 受影响插件。
- 虚拟分身按运行实例 ID 进入备份范围，配置与标准数据目录通过宿主实例接口读写，避免本体与分身串数据。
