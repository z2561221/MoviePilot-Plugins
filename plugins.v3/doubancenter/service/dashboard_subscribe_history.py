...
def same_record(
    ...
    if unique and item.get("unique") == unique:
        return True
    target_source, target_id = legacy_identity(
        media_source=media_source,
        media_id=media_id,
        tmdb_id=tmdbid,
    )
    if target_source and target_id:
        item_source, item_id = identity_from_media(item)
        if item_source == target_source and str(item_id or "") == str(target_id):
...
def dedupe_records(records: list) -> tuple:
    ...
    for record in records:
        ...
        identity_key = (
            str(getattr(source, "value", source) or ""),
            str(media_id or ""),
        )
        if not any(identity_key):
            identity_key = ("legacy", str(record.get("tmdbid") or record.get("tmdb_id") or ""))
        key = (
