...
class DoubanApi:
    ...
    def get_subject_id(self, title: str = None, meta: MetaBase = None) -> Tuple:
        ...
        response = self._request_utils(headers=self.headers).get_res(url)
    ...
    def _get_wish_items_from_pages(self, user_id: str, max_pages: int = 1, days: int = 7, request_get=None, now=None) -> list:
        ...
        getter = request_get
        if getter is None:
            getter = lambda url: self._request_utils(headers=self.headers).get_res(url)
        try:
    ...
    def set_watching_status(self, subject_id: str, status: str = "do", private: bool = True) -> bool:
        ...
        if private:
            data["private"] = "on"
        response = self._request_utils(headers=self.headers).post_res(
