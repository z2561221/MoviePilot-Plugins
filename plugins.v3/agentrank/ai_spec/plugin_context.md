...
本周期开发版本为 `3.0.6`。Telegram 运行异常通知通过持久化一次性会话提供重试按钮，校验原接收人、画像、有效期和最新运行状态，受理后在有界后台线程执行 `telegram_retry` 运行；失败继续通知，停止插件时取消任务。媒体库状态仍使用 `true`、`false`、`null` 区分存在、不存在与查询失败，排除已入库时拦截未知状态。
