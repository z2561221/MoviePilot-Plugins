...
class MediaRecognitionAdapter:
    ...
    _MEDIA_SOURCE_PATTERN = re.compile(r"^[a-z][a-z0-9._-]{0,63}$")
    ...
    def _mapped_tmdb_id(cls, value: Any) -> Optional[int]:
        ...
        if isinstance(value, Mapping):
            source = cls._source_name(value.get("media_source"))
            if source == "tmdb":
                tmdb_id = cls._positive_tmdb_id(value.get("media_id"))
                if tmdb_id:
                    return tmdb_id
        ...
        source = cls._source_name(getattr(value, "media_source", None))
        if source == "tmdb":
            tmdb_id = cls._positive_tmdb_id(getattr(value, "media_id", None))
            if tmdb_id:
                return tmdb_id
    ...
    def _map_source_to_tmdb(
        ...
        """通过 V3 统一身份转换链把跨源身份转换到 TMDB。"""
        if not source or source == "tmdb":
            return cls._positive_tmdb_id(media_id) if source == "tmdb" else None
        try:
            source_value = MediaSource(source)
        except ValueError:
            return None
        try:
            mapping = chain.convert_media_identity(
    ...
    def _recognize_source(
        ...
        """按 V3 成对来源身份调用媒体识别入口。"""
        try:
            source_value = MediaSource(
                str(MediaSource.TMDB) if source == "tmdb" else source
            )
        except ValueError:
            return None
    ...
    def recognize(self, candidate: Candidate) -> Optional[Candidate]:
        ...
        if mapped_tmdb_id:
            mediainfo = self._recognize_source(
                chain, meta, media_type, "tmdb", str(mapped_tmdb_id)
            )
        ...
        if not resolved_tmdb_id:
            return None
        if self._source_name(getattr(mediainfo, "media_source", None)) != "tmdb":
            mediainfo = self._recognize_source(
                chain, meta, media_type, "tmdb", str(resolved_tmdb_id)
            )
            resolved_tmdb_id = self._mapped_tmdb_id(mediainfo)
            if not resolved_tmdb_id:
                return None
    ...
    def enrich_cross_source_ids(self, candidate: Any) -> Any:
        ...
        media_type = self._media_type(candidate, media_type_cls)
        try:
            douban_info = chain.convert_media_identity(
                target_source=MediaSource.Douban,
                media_source=MediaSource.TMDB,
                media_id=str(candidate.source_ids["tmdb"]),
                mtype=media_type,
            )
        except Exception:
            douban_info = None
        if isinstance(douban_info, Mapping):
            douban_id = (
                douban_info.get("media_id")
                or douban_info.get("id")
                or douban_info.get("douban_id")
            )
