...
`v3.0.7`：修正回合创建新 Agent 并显式携带角色快照与提交格式；读取后只开放提交工具，重复读取返回提交提示。首轮和修正回合均接收流式缓冲区中的合法结构化结果。运行历史增加首轮成功、修正成功、终止失败和有限失败分类，并显示缺少有效输出提示。内部受信工具不向普通聊天公开。Telegram 重试仍复用一条消息并实时更新阶段与结果。
...
- Telegram 封面卡保留五条简明推荐和订阅按钮，完整简介见插件详情；摘要通知按渠道使用 HTML 或纯文本列表。
- 运行重试仅接受原接收人操作，有效期为 24 小时；绑定原消息及重试代次，旧回调和迟到进度不会重复运行或覆盖新状态，生成成功前保留旧榜单。编辑失败只重试编辑，不另发通知。
