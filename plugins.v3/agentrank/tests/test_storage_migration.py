...
def test_migration_preserves_old_fixture_field_by_field_and_creates_empty_schemas():
    ...
    assert first.profiles[0].observed == {
        ...
        "identity_storage_count": 1,
    }
    assert second.profiles[0].created_keys == []
    migrated = copy.deepcopy(plugin.data)
    board_key = repository._profile_key("recommendation_board", PROFILE_ID)
    migrated[board_key]["recommendations"][0].pop("media_source")
    migrated[board_key]["recommendations"][0].pop("media_id")
