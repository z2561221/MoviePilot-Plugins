...
const api = {
  async get(path, request = {}) { return { success: true, message: '', data: dataFor(path, request.params || {}) } },
  async post(path, payload = {}) {
    if (path.endsWith('refresh')) {
      status.value = 'running'
      window.setTimeout(() => { status.value = 'success' }, 3500)
      return { success: true, message: '', data: dataFor('run-progress', payload) }
    }
    if (path.endsWith('feedback')) return { success: true, message: '', data: previewFeedback(payload.kind, payload.candidate_id) }
    if (path.endsWith('archive')) return { success: true, message: '', data: previewFeedback('ignore', payload.candidate_id) }
    if (path.endsWith('analysis/comment')) return { success: true, message: '', data: { changed: true, message: '评论已进入异步理解队列' } }
    if (path.endsWith('conversation/messages')) {
      ...
      return { success: true, message: '', data: previewConversation(identities[0]) }
    }
    if (path.endsWith('conversation/messages/retry')) return { success: true, message: '', data: previewConversation(identities[0]) }
    if (path.endsWith('conversation/commands/respond')) {
      ...
      return { success: true, message: '', data: { changed: true } }
    }
    ...
    if (path.endsWith('data/reset/full/prepare')) {
      return { success: true, message: '', data: { confirmation_token: `preview-${Date.now()}`, expires_at: new Date(Date.now() + 300000).toISOString() } }
    }
    if (path.endsWith('data/reset/learning')) return { success: true, message: '', data: { reset: 'learning' } }
    if (path.endsWith('data/reset/full')) return { success: true, message: '', data: { reset: 'full' } }
    return { success: true, message: '', data: { changed: true, message: '预览操作已完成' } }
  },
  async put() { return { success: true, message: '', data: {} } },
}
