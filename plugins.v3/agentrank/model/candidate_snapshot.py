...
CANDIDATE_SNAPSHOT_SCHEMA_VERSION = 4
...
class CandidateSnapshot:
    ...
    def __post_init__(self) -> None:
        ...
        if self.schema_version >= CANDIDATE_SNAPSHOT_SCHEMA_VERSION:
            for candidate in self.candidates:
                if typed_tmdb_candidate_id(candidate.candidate_id) != candidate.candidate_id:
                    raise ValueError("candidate snapshot contains non-canonical TMDB id")
                if candidate.media_source != "themoviedb" or not candidate.media_id:
                    raise ValueError("candidate snapshot contains incomplete media identity")
                if (
                    ...
                    raise ValueError("candidate snapshot media identity mismatch")
