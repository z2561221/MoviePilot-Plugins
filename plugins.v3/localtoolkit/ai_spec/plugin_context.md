...
This context reflects the V3-specific source layout in
`plugins.v3/localtoolkit` and the Vue federation runtime in `dist/assets`.
...
- `plugins.v3/localtoolkit/__init__.py`
...
- `plugins.v3/localtoolkit/plugin.json`
  - Mirrors the plugin metadata used by MoviePilot.
- `package.local.v3.json`
  - Contains the local-only V3 entry; the sync helper materializes it as
    `package.v3.json` in the MoviePilot local plugin repository.

## Backend Structure

- `controller/api.py`
  - Declares concrete V3 response models and shapes single-envelope responses.
...
- `model/api.py`
  - Owns concrete V3 status, history, options, cache, missing-item, and run
    response models.
- `service/lifecycle.py`
...
- `api.js`: injected MoviePilot V3 envelope helper; reads business data from
  `response.data` and never unwraps a second nested `data` field.
...
Frontend build, from `plugins.v3/localtoolkit/frontend`:
...
```powershell
Remove-Item -LiteralPath 'plugins.v3/localtoolkit/frontend/node_modules' -Recurse -Force
Remove-Item -LiteralPath 'plugins.v3/localtoolkit/frontend/pnpm-lock.yaml' -Force
```
