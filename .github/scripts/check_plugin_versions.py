...
V3_EXCEPTIONS_FILE = ".github/v3-version-exceptions.json"
...
def _load_v3_exceptions(root: Path) -> dict:
    """读取 V3 版本例外；缺失文件表示没有例外。"""
    path = root / V3_EXCEPTIONS_FILE
    if not path.exists():
        return {}
    with path.open("r", encoding="utf-8") as file_obj:
...
def _check_v3_release_contract(
    path: Path,
    plugin_id: str,
    meta: dict,
    exceptions: dict,
    ...
    if meta.get("system_version") != ">=3.0.0":
        errors.append(f"{path}: {plugin_id} 缺少 system_version >=3.0.0")
    ...
    exception = exceptions.get(plugin_id)
        ...
        if not isinstance(exception, dict):
            ...
            if package_version != str(exception.get("v3_version") or ""):
                errors.append(
                    ...
                    f"exception={exception.get('v3_version')}, package={package_version}"
                )
    elif parsed_version and legacy_version:
